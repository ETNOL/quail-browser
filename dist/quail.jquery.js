...
var quail = {
  ...
  getConfiguration : function(testName) {
    ...
    return this.accessibilityTests[testName].guidelines[this.options.guidelineName].configuation;
  },
...
quail.colors = {
  ...
  passesWCAG : function(element) {
    return (quail.colors.getLuminosity(quail.colors.getColor(element, 'foreground'), quail.colors.getColor(element, 'background')) > 4.5);
  },
...
quail.aAdjacentWithSameResourceShouldBeCombined = function() {
  quail.html.find('a').each(function() {
    if($(this).next('a').attr('href') === $(this).attr('href') && $(this).attr('href') != undefined && $(this).attr('href') != "#") {
...
(function () {
  var AccessibilityAuditor = {
    audit: function () {
      $('.example').each(function(index, element) {
        $(element).quail({
          jsonPath: '../../Content/Scripts/Vendor/quail',
          guideline: 'wcag',
          complete: function (event) {
            ...
            for (result in results) {
              if (results.hasOwnProperty(result)) {
                if (results[result].elements.length && results[result].test.description) {
                  var level;
                  var className;
                  var testability = results[result].test.testability;
                  if ( testability == 1) {
                    level = 'Severe: ';
            ...
            var header = $('<h1>').addClass("h4").text('Accessibility Results');
            accessDiv.append(header).append(ul);
            $(element).append(accessDiv);
          }
        });
      });
      $('.accessibility').show();
  ...
  $('#test-accessibility').click(function () {
